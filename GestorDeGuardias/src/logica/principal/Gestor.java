...
public class Gestor {
                        ...
                        if ((persona.getDiasDesdeUltimaGuardiaAsignada(fecha) > 31 &&
                            ...
                            if (fechaEsReceso) {
                                if (persona.estaDisponibleEnRecesoDocente(fecha)) {
                                    if (persona.getCantGuardiasDebidas() > 0) {
                                        personasConDeuda.add(persona);
                                    } else {
                                        personasSinDeuda.add(persona);
                                    }
                                }
                            } else if (persona.getCantGuardiasDebidas() > 0) {
                                personasConDeuda.add(persona);
                            } else {
                                personasSinDeuda.add(persona);
                            }
    ...
    public ArrayList<TurnoGuardia> getTurnosPorActualizarCumplimiento(DiaGuardia dia) {
        ArrayList<TurnoGuardia> turnos = dia.getTurnosPorActualizar();
        if (facultad.fechaEsRecesoDocente(dia.getFecha())) {
            for (TurnoGuardia turno : turnos) {
                if (turno.getPersonaAsignada() == null) {
                    turnos.remove(turno);
                }
            }
...
//					turno.getPersonaAsignada().actualizarCumplimiento(dia.getFecha(), turno.getCumplimiento());
    ...
    public void reiniciarCumplimiento(ArrayList<DiaGuardia> dias) throws EntradaInvalidaException, MultiplesErroresException {
        for (DiaGuardia dia : dias)
            for (TurnoGuardia turno : dia.getTurnos())
                turno.actualizarCumplimiento(null);
    }
    // *********************************************************************Filtros y otros**********************************************************************************
    ...
    public ArrayList<Persona> getPersonasDisponiblesConGuardiasDeRecuperacion(LocalDate fecha, HorarioGuardia horario, ArrayList<DiaGuardia> diasEnPantalla) throws MultiplesErroresException, EntradaInvalidaException {
        ...
        while (personasDisponibles.get(i).getCantGuardiasDebidas() > 0) {
    ...
    public void borrarPlanificacion(LocalDate fechaCorte) throws EntradaInvalidaException {
        Iterator<DiaGuardia> iterator = planDeGuardias.iterator();
        if (fechaCorte.getMonthValue() < LocalDate.now().getMonthValue())
            throw new EntradaInvalidaException("No se pueden borrar planificaciones de meses pasados.");
        while (iterator.hasNext()) {
            DiaGuardia dia = iterator.next();
            if (dia.getFecha().isAfter(fechaCorte)) {
                for (TurnoGuardia turno : dia.getTurnos()) {
                    try {
                        Persona persona = facultad.buscarPersona(turno.getPersonaAsignada().getCi());
                        if (persona != null) {
                            persona.eliminarGuardiaAsignada(dia.getFecha());
                        }
                    } catch (Exception e) {
                        //do nothing
                    }
